...
ggplot(df.survival, aes(x = Model, y = Mean)) +
  ...
  geom_errorbar(aes(ymin = Lower_CrI, ymax = Upper_CrI), width = 0.1, size = 1, color = "red2", show.legend = FALSE) + #credible interavals
...
ggplot(df.gam, aes(x = Model, y = Mean, color = "red2")) +
  geom_point(size = 3, show.legend = FALSE, color = "red2") +
  geom_errorbar(aes(ymin = Lower_CrI, ymax = Upper_CrI), width = 0.1, size = 1, show.legend = FALSE, color = "red2") + #credible interavals
